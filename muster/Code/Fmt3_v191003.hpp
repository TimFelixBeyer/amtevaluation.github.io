...
public:
	...
	vector<string> AFInfo;//information on arpeggio and fermata. Used only internally (not written in fmt3 files).

	vector<string> text;//lyrics

	int new_voice;// NOT xml-voice
};//endclass Fmt3Evt

class LessFmt3Evt{
public:
	bool operator()(const Fmt3Evt& a, const Fmt3Evt& b){
		if(a.stime < b.stime){
			return true;
...
};//end class LessFmt3Evt
//stable_sort(Fmt3Evts.begin(), Fmt3Evts.end(), LessFmt3Evt());
...
};//endclass DuplicateOnsetEvtInFmt3
...
public:
	int new_staff;//<- xml-part and xml-staff
	...
	bool operator()(const ExtendedFmt1xEvt& a, const ExtendedFmt1xEvt& b){
		if(a.stime < b.stime){
			...
			if(a.eventtype=="attributes"){
				...
				return false;
			}else if(a.new_staff<b.new_staff){
				return true;
			}else if(a.new_staff>b.new_staff){
				return false;
			}else if(a.voice<b.voice){
...
class StaffCorresp{// staff correspondence: fmt3-staff <-> xml-part, xml-staff
public:
	int fmt3_staff;
	int xml_part;
	int xml_staff;
};//endclass StaffCorresp
...
public:
	...
	vector<DuplicateOnsetEvtInFmt3> duplicateOnsets;
	vector<StaffCorresp> staffCorresps;// staff correspondence: fmt3-staff <-> xml-part, xml-staff
	int TPQN;
		...
		while(ifs>>s[0]){
			if(s[0][0]=='/' || s[0][0]=='#'){
				if(s[0]=="//TPQN:"){
					ifs>>TPQN;
					getline(ifs,s[99]);
				}else if(s[0]=="//Fmt3Version:"){
					ifs>>s[1];
					if(s[1]!="191003"){
//						cout<<"Warning: The fmt3 version is not 191003!"<<endl;
					}//endif
					getline(ifs,s[99]);
				}else if(s[0]=="//DuplicateOnsets:"){
					DuplicateOnsetEvtInFmt3 dup;
					...
					getline(ifs,s[99]);
				}else if(s[0]=="#Fmt3-staff"){
					StaffCorresp staffCorresp;
					ifs>>staffCorresp.fmt3_staff>>s[1]>>s[2]>>staffCorresp.xml_part>>s[3]>>staffCorresp.xml_staff;
					staffCorresps.push_back(staffCorresp);
					getline(ifs,s[99]);
				}else{
					getline(ifs,s[99]);
	...
	void WriteFile(string filename){
		...
		ofs<<"//TPQN: "<<TPQN<<"\n";
		ofs<<"//Fmt3Version: 191003\n";

		for(int i=0;i<staffCorresps.size();i+=1){
			ofs<<"#Fmt3-staff "<<staffCorresps[i].fmt3_staff<<"  =  xml-part: "<<staffCorresps[i].xml_part<<" xml-staff: "<<staffCorresps[i].xml_staff<<"\n";
		}//endfor i
	...
	vector<int> FindFmt3ScorePos(string Id_fmt1,int startPos=0){
		...
		return out;
	}//end FindFmt3ScorePos

	bool IsPitchError(string fmt1ID_,int performedPitch_){
		vector<int> scorePos=FindFmt3ScorePos(fmt1ID_);
	...
	Fmt3 SubScore(int minStime,int maxStime){
		Fmt3 subScore;
		subScore.Clear();
		subScore.TPQN=TPQN;
	...
	void ConvertFromFmt1x(Fmt1x &fmt1){
		...
		for(int i=0;i<fmt1.comments.size();i+=1){
			if(fmt1.comments[i].find("Instrument:")!=string::npos){
				comments.push_back(fmt1.comments[i]);
			}//endif
			if(fmt1.comments[i].find("Part-name:")!=string::npos){
				comments.push_back(fmt1.comments[i]);
			}//endif
		}//endfor i
...

{
		...
		for(int i=0;i<exfmt1.evts.size();i+=1){
			staffCorresp.xml_part=exfmt1.evts[i].part;
			staffCorresp.xml_staff=exfmt1.evts[i].staff;
			int foundpos=-1;
			for(int j=0;j<staffCorresps.size();j+=1){
				if(staffCorresps[j].xml_part==staffCorresp.xml_part && staffCorresps[j].xml_staff==staffCorresp.xml_staff){
					foundpos=j;
					break;
				}//endif
			}//endfor j
			if(foundpos<0){
				foundpos=staffCorresps.size();
				staffCorresp.fmt3_staff=staffCorresps.size();
				staffCorresps.push_back(staffCorresp);
			}//endif
			exfmt1.evts[i].new_staff=foundpos;
		}//endfor i
}//
...
//	cout<<curKeyFifth<<" "<<curKeyMode<<endl;
			...
			nevt.staff=evt.new_staff;
			nevt.voice=evt.voice;
			nevt.new_voice=evt.new_voice;
			nevt.subvoice=evt.subvoice;
			nevt.eventtype=evt.eventtype;
			nevt.dur=evt.dur;
			nevt.sitches.clear(); nevt.notetypes.clear(); nevt.fmt1IDs.clear(); nevt.AFInfo.clear(); nevt.text.clear();
			...
			if(nevt.eventtype=="rest"){
				...
				for(int j=0;j<evt.numNotes;j+=1){
					...
					nevt.text.push_back(evt.text[j]);
...
{
		vector<vector<Fmt3Evt> > voice_nevtSeq(part_voice.size());
		for(int i=0;i<evts.size();i+=1){
			voice_nevtSeq[evts[i].new_voice].push_back(evts[i]);
		}//endfor i

		/// Reduce rests and tremolos and convert short-app after trills to after-notes
		for(int i=0;i<part_voice.size();i+=1){

			if(voice_nevtSeq[i].size()==0){continue;}
		...
		stable_sort(evts.begin(), evts.end(), LessFmt3Evt());
