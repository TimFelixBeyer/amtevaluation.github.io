...
/////////////////////////////////////
			for(int j=0;j<evt.numNotes;j+=1){
				...
				evt.partXML=atoi(s[1].c_str());
				s[0]=s[0].substr(s[0].find_first_of('-')+1,s[0].size());
				s[1]=s[0].substr(0,s[0].find_first_of('-'));
				evt.staffXML=atoi(s[1].c_str());
				s[0]=s[0].substr(s[0].find_first_of('-')+1,s[0].size());
				evt.voiceXML=atoi(s[0].c_str());
			}//endfor j
/////////////////////////////////////
			getline(ifs,s[99]);
			evts.push_back(evt);
...
/////////////////////////////////////
		for(int i=0;i<evts.size();i+=1){
			...
			for(int j=0;j<evts[i].numNotes;j+=1){
				ss.str(""); ss<<evts[i].partXML<<"-"<<evts[i].staffXML<<"-"<<evts[i].voiceXML;
				evts[i].notetypes[j]=ss.str();
			}//endfor j
		}//endfor i
/////////////////////////////////////
		for(int i=0;i<evts.size();i+=1){
...
//	cout<<curKeyFifth<<" "<<curKeyMode<<endl;
			...
			nevt.partXML=evt.part;
			nevt.staffXML=evt.staff;
			nevt.voiceXML=evt.voice;
