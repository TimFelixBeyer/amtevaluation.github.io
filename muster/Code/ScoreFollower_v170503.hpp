			...
			if(Type[i]!="TR"){
				for(int j=0;j<PitchClusters[i].size();j+=1){
					for(int k=0;k<PitchClusters[i][j].size();k+=1){
						...
						if (0 <= PitchClusters[i][j][k] && PitchClusters[i][j][k] < 128) {
							vd[PitchClusters[i][j][k]]+=d[0];
						}
						d[0]=0.015/2.;//semi-tone
						if(VoiceClusters[i][j][k]<0){d[0]/=2.;}//endif
						if (0 <= PitchClusters[i][j][k]+1 && PitchClusters[i][j][k]+1 < 128) {
						...
						if(VoiceClusters[i][j][k]<0){d[0]/=2.;}//endif
						if (0 <= PitchClusters[i][j][k]+2 && PitchClusters[i][j][k]+2 < 128) {
						...
						d[0]=0.0047/2.;//octave
						if(VoiceClusters[i][j][k]<0){d[0]/=2.;}//endif
						if (0 <= PitchClusters[i][j][k]+12 && PitchClusters[i][j][k]+12 < 128) {
						...
						for(int pp=1;pp<12;pp+=1){
							...
							if (0 <= PitchClusters[i][j][k]+pp && PitchClusters[i][j][k]+pp < 128) {
				...
				for(int k=0;k<PitchClusters[i][jj].size();k+=1){
					d[0]=0.95;//chordal
					if (0 <= PitchClusters[i][jj][k] && PitchClusters[i][jj][k] < 128) {
						vd[PitchClusters[i][jj][k]]+=d[0]*factor;
					}
					d[0]=0.015/2.;//semi-tone
					if (0 <= PitchClusters[i][jj][k]+1 && PitchClusters[i][jj][k]+1 < 128) {
					...
					d[0]=0.022/2.;//whole-tone
					if (0 <= PitchClusters[i][jj][k]+2 && PitchClusters[i][jj][k]+2 < 128) {
					...
					d[0]=0.0047/2.;//octave
					if (0 <= PitchClusters[i][jj][k]+12 && PitchClusters[i][jj][k]+12 < 128) {
					...
					for(int pp=1;pp<12;pp+=1){
						if(pp==1||pp==2){continue;}
						if (0 <= PitchClusters[i][jj][k]+pp && PitchClusters[i][jj][k]+pp < 128) {
